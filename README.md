...
> Integrated into this repository from
> [hoainho/img2threejs-showcase](https://github.com/hoainho/img2threejs-showcase)
> at commit `35e063501546c7154216dc5b29366d4a0ae219d5`. See
> [INTEGRATION.md](INTEGRATION.md) for local testing, configuration, and the
> inputs needed to add your own reconstruction.
...
- three ^0.169.0, TypeScript ^5.6.0, Vite ^8.1.5
...
`vite.config.ts` uses a relative base by default, so the build works at the
domain root or under a project subpath. Set `VITE_BASE_PATH` when a host
requires an explicit absolute base.
